	...
	$(function() {
		...
		$("#sortable-system").sortable({
			...
			start: function(event, ui) {
				var startPosition = ui.item.index(); // Note index starts at 1
				ui.item.data('startPos', startPosition);
				console.log("starting pos: " + startPosition);
				console.log(ui.item.data('startPos'));
			},
			
			/*change: function(event, ui) {
				var originPos = ui.item.data('startPos');
			    var currentPos = ui.placeholder.index();
			...
			update: function(event, ui) {
				...
				console.log("UPDATE -- startPos: " + startPosition + " endPos: " + endPosition);
			},
			...
			beforeStop: function(event, ui) { // Before releasing dragging
				if (!inBox) {
					ui.item.remove();
					//numOfNotes--; // Note to self: removes note at back of list in a sense
			...
			receive: function(event, ui) {
				...
				$("#sortable-system div").not(document.getElementById("grid-system")).css({
